...
@require_POST
def distance_result_partial(request: HttpRequest) -> HttpResponse:
    form = DistanceCalculatorForm(request.POST)
    if form.is_valid():
        # Reuse the CBV logic by instantiating and delegating would be heavy; compute here
        lat1, lon1 = form.origin_coords
        lat2, lon2 = form.destination_coords
        unit = form.cleaned_data["unit"].lower()
        route_factor = float(form.cleaned_data["route_factor"])  # already validated
        ...
        return render(request, "calculators/partials/distance_result.html", context)
    return render(request, "calculators/distance_calculator.html", {"form": form}, status=400)


@require_POST
def cost_result_partial(request: HttpRequest) -> HttpResponse:
    ...
    if form.is_valid():
        ...
        return render(request, "calculators/partials/cost_result.html", context)
    return render(request, "calculators/cost_calculator.html", {"form": form}, status=400)
